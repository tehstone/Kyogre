...
async def _configure_welcome(ctx,Kyogre):
    ...
    if config_dict_temp['team']['enabled']:
        welcomeconfig += "Welcome to {server_name}, {owner_name.mention}! Set your team by typing '**!team mystic**' or '**!team valor**' or '**!team instinct**' without quotations. If you have any questions just ask an admin.".format(server_name=guild.name, owner_name=owner)
    else:
        welcomeconfig += 'Welcome to {server_name}, {owner_name.mention}! If you have any questions just ask an admin.'.format(server_name=guild, owner_name=owner)
    ...
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=welcomeconfig).set_author(name='Welcome Message', icon_url=Kyogre.user.avatar_url))
    while True:
        ...
        if welcomereply.content.lower() == 'y':
            ...
            await owner.send(embed=discord.Embed(
                ...
                description=("Would you like a custom welcome message? "
                             ...
                             "Surround your message with [] to send it as an embed. **Warning:** Mentions within embeds may be broken on mobile, this is a Discord bug.")).set_author(name="Welcome Message", icon_url=Kyogre.user.avatar_url))
            if config_dict_temp['welcome']['welcomemsg'] != 'default':
                await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=config_dict_temp['welcome']['welcomemsg']).set_author(name="Current Welcome Message", icon_url=Kyogre.user.avatar_url))
            while True:
                ...
                if welcomemsgreply.content.lower() == 'n':
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description="Please shorten your message to less than 500 characters. You entered {count}.".format(count=len(welcomemsgreply.content))))
                    continue
                else:
            ...
            while True:
                ...
                if welcomechannelreply.content.lower() == "dm":
                    ...
                    if (not diff):
                        ...
                        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Welcome Channel set to {channel}'.format(channel=welcomechannelreply.content.lower())))
                        break
                    else:
...
async def _configure_raid(ctx,Kyogre):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Raid Reporting allows users to report active raids with **!raid** or raid eggs with **!raidegg**. Pokemon raid reports are contained within one or more channels. Each channel will be able to represent different areas/communities. I'll need you to provide a list of channels in your server you will allow reports from in this format: `channel-name, channel-name, channel-name`\n\nExample: `kansas-city-raids, hull-raids, sydney-raids`\n\nIf you do not require raid or raid egg reporting, you may want to disable this function.\n\nRespond with: **N** to disable, or the **channel-name** list to enable, each seperated with a comma and space:").set_author(name='Raid Reporting Channels', icon_url=Kyogre.user.avatar_url))
    citychannel_dict = {}
    while True:
        ...
        if citychannels.content.lower() == 'n':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Raid Reporting disabled'))
            break
            ...
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Raid Reporting Channels enabled'))
    ...
    if config_dict_temp['raid']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to the raid or egg! To do this, I need to know which region each report channel represents using the region names as previously configured (see below), to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding region using only letters and spaces, with each region seperated by a comma and space.\n\nExample: `kanto, johto, sinnoh`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **region name, region name, region name** each matching the order of the previous channel list below.').set_author(name='Raid Reporting Regions', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{region_names}'.format(region_names=(', '.join(region_names)).lower()[:2000])).set_author(name='Entered Regions', icon_url=Kyogre.user.avatar_url))
            while True:
                ...
                if len(region_list) == len(citychannel_list):
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description="The number of regions doesn't match the number of channels you gave me earlier!\n\nI'll show you the two lists to compare:\n\n{channellist}\n{regionlist}\n\nPlease double check that your regions match up with your provided channels and resend your response.".format(channellist=', '.join(citychannel_names), regionlist=', '.join(region_list))))
                    continue
        else:
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to the raid or egg! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='Raid Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
        ...
        while True:
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description="In the same order as they appear below, please give the names of the categories you would like raids reported in each channel to appear in. You do not need to use different categories for each channel, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `kansas city, hull, 1231231241561337813`\n\nYou have configured the following channels as raid reporting channels.").set_author(name='Raid Reporting Categories', icon_url=Kyogre.user.avatar_url))
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
                    ...
                    if len(regioncat_list) == len(citychannel_list):
                        ...
                        if (not catdiff) and (not regioncat_errors):
                            ...
                            msg = "The category list you provided doesn't match with your server's categories."
                            if regioncat_errors:
                ...
                while True:
                    ...
                    for cat in guild.categories:
                        guild_catlist.append(cat.id)
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description="Pokemon Go currently has five levels of raids. Please provide the names of the categories you would like each level of raid to appear in. Use the following order: 1, 2, 3, 4, 5 \n\nYou do not need to use different categories for each level, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `level 1-3, level 1-3, level 1-3, level 4, 1231231241561337813`").set_author(name='Raid Reporting Categories', icon_url=Kyogre.user.avatar_url))
                    ...
                    if len(levelcat_list) == 5:
                        ...
                        msg = "The number of categories I found in your server doesn't match the number of raid levels! Make sure you give me exactly six categories, one for each level of raid. You can use the same category for multiple levels if you want, but I need to see six category names.\n\n**Matched Categories:** {catlist}\n\nPlease double check your categories.".format(catlist=', '.join(levelcat_names) if len(levelcat_list)>0 else "None")
                        if levelcat_errors:
                            msg += "\n\nThe following aren't in your server: **{invalid_categories}**".format(invalid_categories=', '.join(levelcat_errors))
                        await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description=msg))
                        continue
            else:
                await owner.send(embed=discord.Embed(colour=discord.Colour.orange(),description="Sorry, I didn't understand your answer! Try again."))
                continue
            break
        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Raid Categories are set'))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="For each of the regions with raid reporting enabled, please provide the region names\
                ...
                `Johto, Kanto, Hoenn`").set_author(name='Raid Reporting Categories', icon_url=Kyogre.user.avatar_url))
...
async def _configure_exraid(ctx,Kyogre):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="EX Raid Reporting allows users to report EX raids with **!exraid**. Pokemon EX raid reports are contained within one or more channels. Each channel will be able to represent different areas/communities. I'll need you to provide a list of channels in your server you will allow reports from in this format: `channel-name, channel-name, channel-name`\n\nExample: `kansas-city-raids, hull-raids, sydney-raids`\n\nIf you do not require EX raid reporting, you may want to disable this function.\n\nRespond with: **N** to disable, or the **channel-name** list to enable, each seperated with a comma and space:").set_author(name='EX Raid Reporting Channels', icon_url=Kyogre.user.avatar_url))
    citychannel_dict = {}
    while True:
        citychannels = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
        if citychannels.content.lower() == 'n':
            config_dict_temp['exraid']['enabled'] = False
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='EX Raid Reporting disabled'))
            break
        elif citychannels.content.lower() == 'cancel':
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='**CONFIG CANCELLED!**\n\nNo changes have been made.'))
            return None
            ...
            diff = set(citychannel_list) - set(guild_channel_list)
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='EX Raid Reporting Channels enabled'))
                for channel in citychannel_objs:
                    ow = channel.overwrites_for(Kyogre.user)
                    ow.send_messages = True
                    ow.read_messages = True
    ...
    if config_dict_temp['exraid']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            region_names = [name for name in config_dict_temp['regions']['info'].keys()]
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to the raid or egg! To do this, I need to know which region each report channel represents using the region names as previously configured (see below), to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding region using only letters and spaces, with each region seperated by a comma and space.\n\nExample: `kanto, johto, sinnoh`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **region name, region name, region name** each matching the order of the previous channel list below.').set_author(name='EX Raid Reporting Regions', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{region_names}'.format(region_names=region_names[:2000])).set_author(name='Entered Regions', icon_url=Kyogre.user.avatar_url))
            while True:
                regions = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                regions = regions.content.lower().strip()
                if regions == 'cancel':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to EX raids! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='EX Raid Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
                cities = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                if cities.content.lower() == 'cancel':
        ...
        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='EX Raid Reporting Locations are set'))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="How would you like me to categorize the EX raid channels I create? Your options are:\n\n**none** - If you don't want them categorized\n**same** - If you want them in the same category as the reporting channel\n**other** - If you want them categorized in a provided category name or ID".set_author(name='EX Raid Reporting Categories', icon_url=Kyogre.user.avatar_url)))
        while True:
            guild = Kyogre.get_guild(guild.id)
            guild_catlist = []
            for cat in guild.categories:
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description="In the same order as they appear below, please give the names of the categories you would like raids reported in each channel to appear in. You do not need to use different categories for each channel, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `kansas city, hull, 1231231241561337813`\n\nYou have configured the following channels as EX raid reporting channels.").set_author(name='EX Raid Reporting Categories', icon_url=Kyogre.user.avatar_url))
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
                    regioncats = await Kyogre.wait_for('message', check=lambda message: message.guild == None and message.author == owner)
                    if regioncats.content.lower() == "cancel":
                        await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='**CONFIG CANCELLED!**\n\nNo changes have been made.'))
                ...
                continue
            break
        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='EX Raid Categories are set'))
        config_dict_temp['exraid']['category_dict'] = category_dict
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Who do you want to be able to **see** the EX Raid channels? Your options are:\n\n**everyone** - To have everyone be able to see all reported EX Raids\n**same** - To only allow those with access to the reporting channel.").set_author(name='EX Raid Channel Read Permissions', icon_url=Kyogre.user.avatar_url))
        while True:
    ...
    while True:
        ...
        if countersconfigset.content.lower() == 'n':
            ...
            if len(counterlevels) > 0:
                ...
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Automatic Counter Levels set to: {levels}'.format(levels=', '.join((str(x) for x in config_dict_temp['counters']['auto_levels'])))))
                break
            else:
...
async def _configure_wild(ctx,Kyogre):
    ...
    while True:
        ...
        if citychannels.content.lower() == 'n':
            ...
            diff = set(citychannel_list) - set(guild_channel_list)
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Wild Reporting Channels enabled'))
                for channel in citychannel_objs:
                    ow = channel.overwrites_for(Kyogre.user)
                    ow.send_messages = True
                    ow.read_messages = True
    ...
    if config_dict_temp['wild']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to wild spawns! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='Wild Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
                cities = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                if cities.content.lower() == 'cancel':
...
async def _configure_research(ctx,Kyogre):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="Research Reporting allows users to report field research with **!research**. Pokemon **research** reports are contained within one or more channels. Each channel will be able to represent different areas/communities. I'll need you to provide a list of channels in your server you will allow reports from in this format: `channel-name, channel-name, channel-name`\n\nExample: `kansas-city-research, hull-research, sydney-research`\n\nIf you do not require **research** reporting, you may want to disable this function.\n\nRespond with: **N** to disable, or the **channel-name** list to enable, each seperated with a comma and space:").set_author(name='Research Reporting Channels', icon_url=Kyogre.user.avatar_url))
    citychannel_dict = {}
    while True:
        citychannels = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
        if citychannels.content.lower() == 'n':
            config_dict_temp['research']['enabled'] = False
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Research Reporting disabled'))
            break
        elif citychannels.content.lower() == 'cancel':
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='**CONFIG CANCELLED!**\n\nNo changes have been made.'))
            return None
            ...
            diff = set(citychannel_list) - set(guild_channel_list)
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Research Reporting Channels enabled'))
                for channel in citychannel_objs:
                    ow = channel.overwrites_for(Kyogre.user)
                    ow.send_messages = True
                    ow.read_messages = True
    ...
    if config_dict_temp['research']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to field research! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='Research Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
                cities = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                if cities.content.lower() == 'cancel':
...
async def _configure_meetup(ctx,Kyogre):
    ...
    while True:
        ...
        if citychannels.content.lower() == 'n':
            ...
            diff = set(citychannel_list) - set(guild_channel_list)
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Meetup Reporting Channels enabled'))
                for channel in citychannel_objs:
                    ow = channel.overwrites_for(Kyogre.user)
                    ow.send_messages = True
                    ow.read_messages = True
    ...
    if config_dict_temp['meetup']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to meetups! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='Meetup Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
                cities = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                if cities.content.lower() == 'cancel':
        ...
        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Meetup Reporting Locations are set'))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="How would you like me to categorize the meetup channels I create? Your options are:\n\n**none** - If you don't want them categorized\n**same** - If you want them in the same category as the reporting channel\n**other** - If you want them categorized in a provided category name or ID").set_author(name='Meetup Reporting Categories', icon_url=Kyogre.user.avatar_url))
        while True:
            guild = Kyogre.get_guild(guild.id)
            guild_catlist = []
            for cat in guild.categories:
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description="In the same order as they appear below, please give the names of the categories you would like raids reported in each channel to appear in. You do not need to use different categories for each channel, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `kansas city, hull, 1231231241561337813`\n\nYou have configured the following channels as meetup reporting channels.").set_author(name='Meetup Reporting Categories', icon_url=Kyogre.user.avatar_url))
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
                    regioncats = await Kyogre.wait_for('message', check=lambda message: message.guild == None and message.author == owner)
                    if regioncats.content.lower() == "cancel":
                        await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='**CONFIG CANCELLED!**\n\nNo changes have been made.'))
                ...
                continue
            break
        await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Meetup Categories are set'))
...
async def _configure_subscriptions(ctx,Kyogre):
    ...
    while True:
        ...
        if subchs.content.lower() == 'n':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Subscriptions disabled'))
            break
            ...
            if (not diff) and (not sub_list_errors):
                ...
                for channel in sub_list_objs:
                    ...
                    try:
                        ...
                        await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description='I couldn\'t set my own permissions in this channel. Please ensure I have the correct permissions in {channel} using **{prefix}get perms**.'.format(prefix=ctx.prefix, channel=channel.mention)))
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Subscriptions enabled'))
                break
            else:
...
async def _configure_pvp(ctx,Kyogre):
    ...
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="The **!pvp** command allows your users to announce to their friends when they're available for pvp. \
        ...
        Otherwise, just send the names or IDs of the channels you want to allow the **!pvp** command in, separated by commas.").set_author(name='PVP Configuration', icon_url=Kyogre.user.avatar_url))
    while True:
        ...
        if pvpmsg.content.lower() == 'cancel':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='PVP disabled.'))
            break
        else:
            ...
            if (not diff) and (not pvp_list_errors):
                ...
                for channel in pvp_list_objs:
                    ...
                    try:
                        ...
                        await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description='I couldn\'t set my own permissions in this channel. Please ensure I have the correct permissions in {channel} using **{prefix}get perms**.'.format(prefix=ctx.prefix, channel=channel.mention)))
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='PVP enabled'))
                break
            else:
...
").set_author(name='Join Link Configuration', icon_url=Kyogre.user.avatar_url))
    while True:
...
async def _configure_lure(ctx,Kyogre):
    ...
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), 
        description="Lure Reporting allows users to report lures they've applied with **!lure**. **lure** are contained within one or more channels. Each channel will be able to represent different areas/communities. I'll need you to provide a list of channels in your server you will allow reports from in this format: `channel-name, channel-name, channel-name`\n\nExample: `kansas-city-lures, hull-lures, sydney-lures`\n\nIf you do not require **lure** reporting, you may want to disable this function.\n\nRespond with: **N** to disable, or the **channel-name** list to enable, each seperated with a comma and space:").set_author(name='Lure Reporting Channels', icon_url=Kyogre.user.avatar_url))
    citychannel_dict = {}
    while True:
        citychannels = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
        if citychannels.content.lower() == 'n':
            config_dict_temp['lure']['enabled'] = False
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Lure Reporting disabled'))
            break
        elif citychannels.content.lower() == 'cancel':
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='**CONFIG CANCELLED!**\n\nNo changes have been made.'))
            return None
            ...
            diff = set(citychannel_list) - set(guild_channel_list)
            if (not diff) and (not citychannel_errors):
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Lure Reporting Channels enabled'))
                for channel in citychannel_objs:
                    ow = channel.overwrites_for(Kyogre.user)
                    ow.send_messages = True
                    ow.read_messages = True
    ...
    if config_dict_temp['lure']['enabled']:
        if config_dict_temp.get('regions', {}).get('enabled', None):
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='For each report, I generate Google Maps links to give people directions to lures! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.').set_author(name='Lure Reporting Locations', icon_url=Kyogre.user.avatar_url))
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description='{citychannel_list}'.format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name='Entered Channels', icon_url=Kyogre.user.avatar_url))
            while True:
                cities = await Kyogre.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
                if cities.content.lower() == 'cancel':
...
async def _configure_archive(ctx,Kyogre):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="The **!archive** command marks temporary raid channels for archival rather than deletion. This can be useful for investigating potential violations of your server's rules in these channels.\n\nIf you would like to disable this feature, reply with **N**. Otherwise send the category you would like me to place archived channels in. You can say **same** to keep them in the same category, or type the name or ID of a category in your server.").set_author(name='Archive Configuration', icon_url=Kyogre.user.avatar_url))
    ...
    while True:
        ...
        if archivemsg.content.lower() == 'same':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Archived channels will remain in the same category.'))
            break
        if archivemsg.content.lower() == 'n':
            config_dict_temp['archive']['enabled'] = False
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Archived Channels disabled.'))
            break
        else:
            ...
            if not category:
                ...
                category = discord.utils.get(guild.categories, name=name)
            if not category:
                await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description="I couldn't find the category you replied with! Please reply with **same** to leave archived channels in the same category, or give the name or ID of an existing category."))
                continue
            config_dict_temp['archive']['category'] = category.id
            config_dict_temp['archive']['enabled'] = True
            await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Archive category set.'))
            break
    if config_dict_temp['archive']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="I can also listen in your raid channels for words or phrases that you want to trigger an automatic archival. For example, if discussion of spoofing is against your server rules, you might tell me to listen for the word 'spoofing'.\n\nReply with **none** to disable this feature, or reply with a comma separated list of phrases you want me to listen in raid channels for.").set_author(name='Archive Configuration', icon_url=Kyogre.user.avatar_url))
        ...
        if phrasemsg.content.lower() == 'none':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Phrase list disabled.'))
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Archive Phrase list set.'))
    ctx.config_dict_temp = config_dict_temp
    return ctx
...
async def _configure_settings(ctx,Kyogre):
    ...
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="There are a few settings available that are not within **!configure**. \
        ...
        **!set pokebattler <ID>** - To set a trainer's pokebattler ID (usable by members)\n\n\
        ...
        .format(utctime=strftime('%H:%M', time.gmtime()))).set_author(name='Timezone Configuration and Other Settings', icon_url=Kyogre.user.avatar_url))
    skipped = False
    while True:
...
async def _configure_trade(ctx,Kyogre):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="The **!trade** command allows your users to organize and coordinate trades. This command requires at least one channel specifically for trades.\n\nIf you would like to disable this feature, reply with **N**. Otherwise, just send the names or IDs of the channels you want to allow the **!trade** command in, separated by commas.").set_author(name='Trade Configuration', icon_url=Kyogre.user.avatar_url))
    while True:
        ...
        if trademsg.content.lower() == 'cancel':
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description='Trade disabled.'))
            break
        else:
            ...
            if (not diff) and (not trade_list_errors):
                ...
                for channel in trade_list_objs:
                    ...
                    try:
                        ...
                        await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description='I couldn\'t set my own permissions in this channel. Please ensure I have the correct permissions in {channel} using **{prefix}get perms**.'.format(prefix=ctx.prefix, channel=channel.mention)))
                await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Pokemon Trades enabled'))
                break
            else:
...
async def _get_listings(ctx, Kyogre, guild, owner, config_dict_temp):
    ...
    if config_dict_temp.get('regions', {}).get('enabled', None):
        ...
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description="I can also provide listings per region that I will keep updated automatically as events are reported, updated, or expired. To get started, please provide a comma-separated list of channel names, one per region, matching the format of this list of regions:\n\n`{region_list}`\n\n**IMPORTANT** I recommend you set the permissions for each channel provided to allow only me to post to it. I will moderate each channel to remove other messages, but it will save me some work!".format(region_list=', '.join(region_names))).set_author(name='Listing Channels', icon_url=Kyogre.user.avatar_url))
        while True:
            ...
            if listing_channels == 'n':
                ...
                if len(channel_objs) != len(region_names):
                    await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description="The channel list you provided doesn't match with your region list.\n\nPlease provide a channel for each region in your region list:\n\n{region_list}".format(region_list=', '.join(region_names))))
                    continue
                diff = set([x.id for x in channel_objs]) - set(guild_channel_list)
                if (not diff) and (not channel_errors):
                    await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Listing Channels enabled'))
        ...
        while True:
            ...
            if listing_channels == 'n':
                ...
                diff = set([x.id for x in channel_objs]) - set(guild_channel_list)
                if (not diff) and (not channel_errors):
                    await owner.send(embed=discord.Embed(colour=discord.Colour.green(), description='Listing Channel enabled'))
                    for i, channel in enumerate(channel_objs):
                        ow = channel.overwrites_for(Kyogre.user)
                        ow.send_messages = True
                        ow.read_messages = True
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.orange(), description="The channel you provided doesn't match with your servers channels.\n\nPlease double check your channel and resend your reponse."))
                    continue
    return listing_dict
